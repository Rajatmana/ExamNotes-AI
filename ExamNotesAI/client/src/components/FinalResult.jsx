...
function FinalResult({ result }) {
  ...
  return (
    <div className="mt-6 p-3 space-y-0 bg-white">
       ...
       <section>
        ...
        {result.charts && result.charts.length === 0 && (
          <p className="text-sm text-gray-400 italic">
            📈 Charts are not relavant for this topic.
          </p>
        )}
...
function SectionHeader({ icon, title, color }) {
  ...
  return (
    <div
      className={`
      mb-4 px-4 py-2 rounded-lg bg-linear-to-r ${colors[color]} font-semibold flex items-center gap-2
      `}
    >
      <span>{icon}</span>
      <span>{title}</span>
